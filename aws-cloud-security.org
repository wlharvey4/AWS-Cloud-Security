...
{{{heading(Access Points---Endpoints)}}}
...
{{{heading(Amazon Virtual Private Cloud)}}}
...
This done using NACLs, /network access control lists/.  These lists contain IPs which are allowed
or denied access to the subnet.  They are also stateless, so if you allow something in, you must
explicitly allow it out via a rule.
